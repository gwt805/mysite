...
def uploadimg(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'errno': 0,  'data': {'url': f"http://{getip()}/blogs/{uid}-{files.name}", "alt": f"{uid}-{files.name}"}})#{getip()}
        except Exception as e:
...
def uploadvideo(request):
    if request.method == 'POST':
        try:
            ...
            uid = str(uuid1())
            upload_file(f"{uid}-{files.name}", files, len(files))
            return JsonResponse({'errno': 0,  'data': {'url': f"http://{getip()}/blogs/{uid}-{files.name}"}})
        except Exception as e:
            return JsonResponse({'errno': 1,  'message': "上传失败"})