...
const editorConfig: Partial<IEditorConfig> = {
    ...
    MENU_CONF: {
        ...
        uploadImage: {
            server: `${API_BASE_URL}/blog/uploadimg/`, // 上传接口 URL
        ...
        uploadVideo: {
            server: `${API_BASE_URL}/blog/uploadother/`, // 上传接口 URL
            ...
            onSuccess(file: File, res: any) {
                console.log(`${file.name} 上传成功`, res);
                const { url } = res.data || {}
                edit_video_list.value.add(url.split("/")[url.split("/").length - 1]);
            },
        ...
        uploadAttachment: {
            server: `${API_BASE_URL}/blog/uploadother/`,
            fieldName: "file",
            withCredentials: true,
            allowedFileTypes: ['*'],
...
const submit = () => {
    ...
    createblog(title.value.trim(), editorRef.value.getHtml(), get_filelist()).then((res)=>{
        if (res.code == 0){
